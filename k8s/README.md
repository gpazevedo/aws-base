...
export SERVICE_NAME="<YOUR-PROJECT>-api"
export ENVIRONMENT="dev"
export IMAGE_URI="123456789012.dkr.ecr.us-east-1.amazonaws.com/<YOUR-PROJECT>:api-dev-latest"
...
kubectl get svc <YOUR-PROJECT>-api -n dev \
...
```bash
# Get endpoint
ENDPOINT=$(kubectl get svc <YOUR-PROJECT>-api -n dev \
  -o jsonpath='{.status.loadBalancer.ingress[0].hostname}')
...
helm install aws-load-balancer-controller eks/aws-load-balancer-controller \
  -n kube-system \
  --set clusterName=<YOUR-PROJECT>-dev \
...
| `SERVICE_NAME` | `<YOUR-PROJECT>-api` | Service name |
| `ENVIRONMENT` | `dev` | Environment (dev/test/prod) |
| `IMAGE_URI` | `123456789012.dkr.ecr.us-east-1.amazonaws.com/<YOUR-PROJECT>:api-dev-latest` | Container image URI |
...
# Specific pod
kubectl logs -n dev <YOUR-PROJECT>-api-xxxxx-yyyyy -f
```
...
```bash
# Scale to 5 replicas
kubectl scale deployment <YOUR-PROJECT>-api -n dev --replicas=5
```
...
```bash
# Update to new image
kubectl set image deployment/<YOUR-PROJECT>-api \
  -n dev \
  api=123456789012.dkr.ecr.us-east-1.amazonaws.com/<YOUR-PROJECT>:api-dev-20250120
...
kubectl rollout history deployment/<YOUR-PROJECT>-api -n dev

# Rollback to previous version
kubectl rollout undo deployment/<YOUR-PROJECT>-api -n dev

# Rollback to specific revision
kubectl rollout undo deployment/<YOUR-PROJECT>-api -n dev --to-revision=2
```
...
```bash
# Exec into pod
kubectl exec -it <YOUR-PROJECT>-api-xxxxx-yyyyy -n dev -- /bin/sh

# Port forward to local machine
kubectl port-forward -n dev svc/<YOUR-PROJECT>-api 8000:80
...
kubectl describe hpa <YOUR-PROJECT>-api -n dev
```
...
```bash
# Check pod status
kubectl describe pod <YOUR-PROJECT>-api-xxxxx-yyyyy -n dev
...
kubectl get endpoints <YOUR-PROJECT>-api -n dev

# Check if LoadBalancer is created
kubectl describe svc <YOUR-PROJECT>-api -n dev

# Test from within cluster
kubectl run test-pod -n dev --rm -it --image=curlimages/curl -- \
  curl http://<YOUR-PROJECT>-api/health
```
...
kubectl run load-generator -n dev --rm -it --image=busybox -- \
  /bin/sh -c "while true; do wget -q -O- http://<YOUR-PROJECT>-api; done"
```
