...
.PHONY: help bootstrap-create bootstrap-init bootstrap-plan bootstrap-apply bootstrap-output setup-terraform-backend setup-terraform-lambda setup-terraform-apprunner \
...
# API Key configuration (defaults to true)
API_KEY ?= true
...
help:
	...
	@echo "  make setup-terraform-lambda    Generate example Lambda Terraform files (API_KEY=true)"
	@echo "  make setup-terraform-apprunner Generate example App Runner Terraform files (API_KEY=true)"
	@echo "  make setup-pre-commit        Setup pre-commit hooks (Ruff + Pyright)"
	@echo "  API_KEY=false make setup-terraform-lambda    Disable API Key auth"
	@echo ""
...
setup-terraform-lambda:
	...
	./scripts/setup-terraform-lambda.sh $(API_KEY)

setup-terraform-apprunner:
	@echo "🏗️  Generating example App Runner Terraform files..."
	./scripts/setup-terraform-apprunner.sh $(API_KEY)
...
# Usage: make test-lambda-api, make test-lambda-worker, etc.
test-lambda-%:
