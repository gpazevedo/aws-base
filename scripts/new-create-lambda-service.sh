...
from app.config import SERVICE_NAME, SERVICE_VERSION, ENVIRONMENT, LOG_LEVEL
...
# Assemble Routers
# We combine them into a single router first to easily mount it twice (root + prefix)
api_router = APIRouter()
api_router.include_router(system.router)

app.include_router(api_router)  # Root access
app.include_router(api_router, prefix=f"/{SERVICE_NAME}")  # API Gateway prefix access

# =============================================================================
# Lambda Handler (Python 3.14 Compatible)
# =============================================================================
try:
    import asyncio
...
def handler(event, context):
    if not _mangum_available:
        raise RuntimeError("Lambda handler not available - mangum package not installed")
...
echo "   $SERVICE_DIR/app/system.py"
