...
echo "🚀 Setting up Lambda service Terraform configuration..."
echo ""
...
if [ ! -f "$LAMBDA_VARS_FILE" ]; then
  echo "📝 Creating terraform/lambda-variables.tf..."
...
# }
EOF
  echo "✅ Created terraform/lambda-variables.tf"
else
  echo "ℹ️  Lambda variables file already exists (terraform/lambda-variables.tf)"
fi
...
for ENV in "${ENVIRONMENTS[@]}"; do
  ...
  if [ -f "$TFVARS_FILE" ]; then
    ...
    if ! grep -q "^lambda_memory_size" "$TFVARS_FILE" 2>/dev/null; then
      echo "📝 Adding Lambda variables to $TFVARS_FILE..."
...
EOF
      ...
      echo "ℹ️  Lambda variables already present in $TFVARS_FILE"
    fi
  fi
done
...
if [ -f "$LAMBDA_TF_FILE" ]; then
  echo "⚠️  Warning: ${LAMBDA_TF_FILE} already exists"
...
echo ""
echo "✅ Lambda service '${SERVICE_NAME}' Terraform configuration created successfully!"
echo ""
echo "📂 Created/Updated files:"
echo "   terraform/lambda-variables.tf"
echo "   terraform/lambda-${SERVICE_NAME}.tf ✨"
...
echo ""
echo "💡 To configure service-specific settings (memory, timeout, etc.):"
