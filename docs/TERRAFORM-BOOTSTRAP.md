...
```hcl
project_name = "<YOUR-PROJECT>"       # Your project name
github_org   = "<YOUR-ORG>"       # GitHub org/username
github_repo  = "<YOUR-PROJECT>"       # GitHub repository
...
**Result:**
- Creates one repository: `{project_name}` (e.g., `<YOUR-PROJECT>`)
...
|----------|------|-------------|---------|
| `project_name` | string | Project identifier (used in resource names) | `"<YOUR-PROJECT>"` |
| `aws_region` | string | Primary AWS region | `"us-east-1"` |
| `aws_account_id` | string | AWS account ID | `"123456789012"` |
| `github_org` | string | GitHub organization/user | `"<YOUR-ORG>"` |
| `github_repo` | string | GitHub repository name | `"<YOUR-PROJECT>"` |
...
# Output: <YOUR-PROJECT>-terraform-state
...
# Output: arn:aws:iam::123456789012:role/<YOUR-PROJECT>-github-actions-lambda
...
terraform {
  backend "s3" {
    bucket  = "<YOUR-PROJECT>-terraform-state"  # From bootstrap output
  ...
  "Statement": [
    {
      ...
      "Condition": {
        ...
        "StringLike": {
          "token.actions.githubusercontent.com:sub": "repo:<YOUR-ORG>/<YOUR-PROJECT>:*"
        }
      }
    }
  ]
...
2. **Backup state files:**
   ```bash
   aws s3 sync s3://<YOUR-PROJECT>-terraform-state ./state-backup/
   ```
...
**Error:**
```
Error: creating Amazon S3 Bucket (<YOUR-PROJECT>-terraform-state): BucketAlreadyExists
```
...
# Edit terraform.tfvars:
state_bucket_name = "<YOUR-PROJECT>-terraform-state-v2"

# Option 2: Import existing bucket
terraform import aws_s3_bucket.terraform_state <YOUR-PROJECT>-terraform-state
```
...
aws iam list-attached-role-policies \
  --role-name <YOUR-PROJECT>-eks-node-group-role

# Check CloudWatch logs for node group
aws eks describe-nodegroup \
  --cluster-name <YOUR-PROJECT>-cluster \
...
# Verify trust policy
aws iam get-role --role-name <YOUR-PROJECT>-github-actions-lambda
